...
<body>
    ...
    <div class="space">
        <div class="" style="background-color: white; width: 400px; overflow: auto; padding: 50px 0; margin: 0 auto; text-align: center; border: 1px solid #f1f1f1; border-radius: 7px;">
            ...
            <?php if ($row_DatosReg["course_1"] != "") { ?>
            Kurs 1: <?php echo ObtenerNombreCurso($row_DatosReg["course_1"]); ?><br>
            <?php } ?>
            <?php if ($row_DatosReg["course_2"] != "") { ?>
            Kurs 2: <?php echo ObtenerNombreCurso($row_DatosReg["course_2"]); ?><br>
            <?php } ?>
            <?php if ($row_DatosReg["course_3"] != "") { ?>
            Kurs 3: <?php echo ObtenerNombreCurso($row_DatosReg["course_3"]); ?><br>
            <?php } ?>
            <?php if ($row_DatosReg["course_4"] != "") { ?>
            Kurs 4: <?php echo ObtenerNombreCurso($row_DatosReg["course_4"]); ?><br>
            <?php } ?>
            <br><br>
            <form action="https://www.paypal.com/cgi-bin/webscr" method="post" id="paypal_form">
                ...
                <input type="hidden" name="cancel_return" value="http://www,tiendazapatos.com/carrito_ko.php?control=<?php //echo $_SESSION["compraactivavisa"]; ?>" />
